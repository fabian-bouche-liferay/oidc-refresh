package com.liferay.samples.fbo.oidc.refresh.token;
...
public class CustomOpenIdConnectServiceHandlerImpl implements OpenIdConnectServiceHandler {

	private static final Logger LOG = LoggerFactory.getLogger(CustomOpenIdConnectServiceHandlerImpl.class);

	private static final String REFRESH_TOKEN_REQUESTED_FLAG = "refresh-token-requested-flag"; 
	...
	protected boolean hasValidAccessToken(
		...
		LOG.debug("Should we anticipate the token refresh?");

		LOG.debug("Current time: " + currentTime);
		LOG.debug("Login time: " + loginTime);
		LOG.debug("Time elapsed: " + (currentTime - loginTime));
		LOG.debug("Lifetime: " + lifetime);
		
		if ((currentTime - loginTime) > lifetime / 2) {
			shouldAnticipateRefresh(openIdConnectSessionImpl);
		} else {
			LOG.debug("No, we should not anticipate the token refresh");
		}
	...
	private void shouldAnticipateRefresh(OpenIdConnectSessionImpl openIdConnectSessionImpl) {

		LOG.debug("We should anticipate the token refresh");
		
		boolean shouldAnticipate = false;
		
		synchronized (openIdConnectSessionImpl) {
			// We should add a flag in the OpenIdConnectSession interface so that I do not savagely put this information in the user info object
			if(openIdConnectSessionImpl.getUserInfo().getClaim(REFRESH_TOKEN_REQUESTED_FLAG) == null || !(Boolean)openIdConnectSessionImpl.getUserInfo().getClaim(REFRESH_TOKEN_REQUESTED_FLAG)) {

				openIdConnectSessionImpl.getUserInfo().setClaim(REFRESH_TOKEN_REQUESTED_FLAG, Boolean.TRUE);
				shouldAnticipate = true;

				LOG.debug("Locking token refresh");
		...
		if(shouldAnticipate) {
			...
			LOG.debug("No thread asked to refresh it, we will do it");

			try {

				refreshAuthToken(openIdConnectSessionImpl);
				...
				openIdConnectSessionImpl.getUserInfo().setClaim(REFRESH_TOKEN_REQUESTED_FLAG, Boolean.FALSE);
			...
			LOG.debug("But another thread has already requested the refresh");
	...
	public void processAuthenticationResponse(
		...
		LOG.debug("UPDATING SESSION TOKENS");
	...
	protected boolean refreshAuthToken(
		...
		// This check makes no sense because you won't go into refreshAuthToken unsell hasValidAccessToken was false
		// Now, with my update, I need to remove it because I want to anticipate token renewal
		/*
		if (hasValidAccessToken(openIdConnectSessionImpl)) {
			return true;
		}
